<style>
    ...
    /* EFEK KARTU TERANGKAT / NAIK SAAT HOVER KURSOR */
    .card-artikel {
...
<section id="artikel" class="bg-[#F7F0E7] py-24">
    <div class="max-w-7xl mx-auto px-8">
        ...
        <div class="filter-tab-container flex gap-10 border-b border-[#DCCCB4] relative">
            <button type="button" data-kategori="semua" class="filter-tab-btn tab-btn-action tab-active pb-4 text-xs tracking-[2px] uppercase font-bold text-[#992B20]">Semua</button>
            <button type="button" data-kategori="ajaran" class="filter-tab-btn tab-btn-action pb-4 text-xs tracking-[2px] uppercase font-medium text-[#8C7A65] hover:text-[#992B20]">Ajaran Tetua</button>
            <button type="button" data-kategori="cecimpedan" class="filter-tab-btn tab-btn-action pb-4 text-xs tracking-[2px] uppercase font-medium text-[#8C7A65] hover:text-[#992B20]">Cecimpedan</button>
            <button type="button" data-kategori="satua" class="filter-tab-btn tab-btn-action pb-4 text-xs tracking-[2px] uppercase font-medium text-[#8C7A65] hover:text-[#992B20]">Satua Bali</button>
            <button type="button" data-kategori="istilah" class="filter-tab-btn tab-btn-action pb-4 text-xs tracking-[2px] uppercase font-medium text-[#8C7A65] hover:text-[#992B20]">Istilah Bali</button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-10">
            ...
            @forelse($artikels as $item)
                @php
                    ...
                    $tanggal = $item->created_at ? \Carbon\Carbon::parse($item->created_at)->translatedFormat('d F Y') : date('d F Y');
                    $ringkasan = \Illuminate\Support\Str::limit(strip_tags($isiText), 110);
                @endphp

                <!-- CARD ARTIKEL (Ditambahkan kelas card-artikel untuk efek hover terangkat) -->
                <div class="card-artikel js-open-modal {{ $katClass }} bg-white rounded-xl overflow-hidden shadow duration-300 cursor-pointer group flex flex-col justify-between"
                    ...
                    <div>
                        ...
                        <div class="relative overflow-hidden">
                            ...
                            <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300 transform group-hover:scale-100 scale-95 z-10">
                                <span class="bg-[#FAF5ED] text-[#2B1A12] border border-[#D6C5AE] text-[11px] font-bold tracking-[2px] uppercase px-5 py-2.5 rounded-lg shadow-lg">
                                    BACA ARTIKEL
                                </span>
                            </div>

                            <!-- Badge Warna Sesuai Kategori -->
                            <span class="absolute top-4 left-4 {{ $badgeBg }} text-white text-[11px] tracking-[2px] uppercase font-semibold px-4 py-2 rounded-full z-10 shadow-sm">
                                {{ strtoupper($kategoriText) }}
                            </span>

                            <span class="absolute top-4 right-4 bg-white/90 text-[#B8863B] text-[10px] tracking-[1px] font-bold px-3 py-1 rounded-full z-10 shadow-sm">
                                Terverifikasi
                            </span>
                        </div>
                        ...
                        <!-- Judul dan Ringkasan -->
                        <div class="p-6">
                            <h3 style="font-family:'Cormorant Garamond',serif;" class="text-[28px] leading-snug font-bold text-[#2B1A12] group-hover:text-[#B8863B] transition line-clamp-2">
                                {{ $judulText }}
                            </h3>
                    ...
                    <!-- Footer Kartu -->
                    <div class="px-6 pb-6 pt-2 flex items-center justify-between text-[13px] text-[#8C7A65] border-t border-[#F3EBE0]">
                        <span>{{ $tanggal }} &nbsp;•&nbsp; 5 Menit</span>

                        <button type="button" class="w-9 h-9 rounded-full border border-[#DCCCB4] flex items-center justify-center text-[#B8863B] hover:bg-[#B8863B] hover:text-white transition">
...
<div id="overlayArtikel" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 hidden opacity-0 transition-opacity duration-300">
    <div id="panelArtikel" class="fixed inset-y-0 right-0 w-full max-w-2xl bg-[#FAF6F0] shadow-2xl p-8 overflow-y-auto transform translate-x-full transition-transform duration-300 ease-in-out border-l border-[#D6C5AE]">
        ...
        <button id="closeModalBtn" type="button" onclick="closeArtikelModal()" class="absolute top-6 right-6 w-10 h-10 rounded-full bg-[#EFE6D8] text-[#2B1A12] flex items-center justify-center hover:bg-[#992B20] hover:text-white transition duration-300 font-bold">✕</button>
        
        <div class="mb-4">
            <span id="artKategoriBadge" class="text-white text-[11px] tracking-[2px] uppercase font-semibold px-4 py-2 rounded-full">KATEGORI</span>
        </div>
        ...
        <div class="rounded-xl overflow-hidden mb-6 shadow-md bg-[#EFE6D8]">
            <img id="artImage" src="" onerror="this.onerror=null;this.src='{{ asset('images/subak.jpeg') }}';" class="w-full h-64 object-cover" alt="Gambar Artikel">
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Modal Event Detail Artikel
        document.querySelectorAll('.js-open-modal').forEach(card => {
            card.addEventListener('click', function (e) {
                if (e.target.closest('button') && !e.target.closest('button').classList.contains('js-open-modal')) {
                    return;
                }

                const ds = this.dataset;
                const isiHtml = this.querySelector('.hidden-isi-content') ? this.querySelector('.hidden-isi-content').innerHTML : '';
