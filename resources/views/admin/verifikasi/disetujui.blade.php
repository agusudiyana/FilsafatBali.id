...
<div class="max-w-7xl mx-auto px-4 py-4 flex flex-col h-[calc(100vh-100px)] text-[#1A110A]">
    ...
    <!-- Header Halaman & Dropdown Filter (Layout Rapi Bebas Benturan) -->
    <div class="flex-none mb-6 border-b border-[#E6D5B8] pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-[#1A110A]">
                Konten Disetujui (Terpublikasi)
            </h1>
            <p class="text-gray-500 text-xs sm:text-sm mt-1">
                Daftar semua kategori ajaran/konten yang telah disetujui dan tayang di platform.
            </p>
        </div>

        <div class="flex items-center gap-3 self-start md:self-auto">
            <!-- Dropdown Filter Modul (Teks Label Sudah Dihapus) -->
    ...
    <div class="bg-white flex-1 rounded-2xl shadow-sm border border-[#E6D5B8] overflow-hidden flex flex-col min-h-0">
        <div class="overflow-y-auto overflow-x-auto flex-1 bg-white">
            <table class="w-full text-left border-collapse bg-white" id="tabelDisetujui">
                ...
                <tbody class="divide-y divide-gray-100 bg-white text-sm">
                    @forelse($semuaDisetujui as $item)
                        <tr class="hover:bg-[#FAF6F0] transition bg-white baris-konten" data-modul="{{ $item->kategori_modul }}">
                            ...
                            <td class="p-4 text-center">
                                <!-- TOMBOL HAPUS -->
                                <form action="{{ route('admin.verifikasi.hapus-konten', ['type' => $item->kategori_modul, 'id' => $item->id]) }}" 
                                      method="POST" 
                                      onsubmit="return confirm('Apakah Anda yakin ingin menghapus data disetujui ini dari platform?');" 
                                      class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                                            title="Hapus Konten"
                                            class="bg-rose-500 hover:bg-rose-600 text-white px-3 py-1.5 rounded-xl transition shadow-sm text-xs font-semibold inline-flex items-center gap-1">
                    ...
                    @empty
                        <tr id="pesanKosongBawaan">
                            <td colspan="6" class="text-center p-8 text-gray-500 bg-white">
                                <div class="flex flex-col items-center justify-center gap-2">
                                    <i data-feather="inbox" class="w-8 h-8 text-gray-400"></i>
                                    <span>Belum ada konten yang disetujui.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                    
                    <!-- Pesan saat filter tidak menemukan data -->
                    <tr id="pesanKosongFilter" class="hidden">
                        <td colspan="6" class="text-center p-8 text-gray-500 bg-white">
                            <div class="flex flex-col items-center justify-center gap-2">
                                <i data-feather="search" class="w-8 h-8 text-gray-400"></i>
                                <span>Tidak ada konten disetujui pada kategori modul ini.</span>
                            </div>
                        </td>
                    </tr>
                </tbody>
...
<script>
    ...
    function filterModulTabel() {
        ...
        const pesanKosongFilter = document.getElementById('pesanKosongFilter');
        let counter = 1;
        ...
        if (pesanKosongFilter) {
            if (!adaDataTampil && barisKonten.length > 0) {
                pesanKosongFilter.classList.remove('hidden');
            } else {
                pesanKosongFilter.classList.add('hidden');
            }
        }
    }
</script>
