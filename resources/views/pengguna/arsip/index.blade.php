<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <h2 class="font-bold text-2xl text-[#2B1A0E] tracking-tight leading-tight"
    ...
    <div class="py-10 bg-[#FAF6F0] min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if (session('success'))
                <div class="bg-[#EFE4D3] border border-[#C8A45A] text-[#8D2B1D] px-5 py-3.5 rounded-2xl relative text-sm font-semibold flex items-center justify-between shadow-sm"
                    role="alert">
                    <span class="block sm:inline">✨ {{ session('success') }}</span>
                    <button type="button" onclick="this.parentElement.remove()"
                        class="text-[#8D2B1D] font-bold text-lg hover:opacity-75">&times;</button>
                </div>
            @endif
            ...
            <div
                ...
                <div class="relative w-full md:w-1/2">
                    <input type="text" id="searchInput" oninput="filterArsip()"
                        placeholder="Ketik huruf awal atau judul naskah..."
                        class="w-full pl-10 pr-4 py-2.5 rounded-xl border-[#E5D6BF] text-sm focus:ring-[#8D2B1D] focus:border-[#8D2B1D] bg-[#FAF6F0]/50">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-[#8C7A65] absolute left-3.5 top-3.5"
                ...
                <div class="relative w-full md:w-auto">
                    <select id="categorySelect" onchange="filterArsip()"
                        class="w-full md:w-auto rounded-xl border-[#E5D6BF] text-sm focus:ring-[#8D2B1D] focus:border-[#8D2B1D] bg-[#FAF6F0]/50 py-2.5 pl-4 pr-10 font-semibold text-[#2B1A0E]">
            ...
            @if (count($items) > 0)
                <div id="arsipGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($items as $arsip)
                        @php
                            $title = is_array($arsip)
                                ? $arsip['judul'] ?? ($arsip['item_title'] ?? 'Tanpa Judul')
                                : $arsip->item_title ?? ($arsip->judul ?? 'Tanpa Judul');
                            $category = is_array($arsip)
                                ? $arsip['kategori'] ?? ($arsip['item_type'] ?? 'Artikel')
                                : $arsip->item_type ?? ($arsip->kategori ?? 'Artikel');
                            $penulis = is_array($arsip) ? $arsip['penulis'] ?? 'Penulis' : $arsip->penulis ?? 'Penulis';
                            $deskripsi = is_array($arsip)
                                ? $arsip['deskripsi'] ?? 'Naskah kebudayaan dan filsafat Bali.'
                                : $arsip->deskripsi ?? 'Naskah kebudayaan dan filsafat Bali.';
                            $dbUrl = is_array($arsip)
                                ? $arsip['url'] ?? ($arsip['item_url'] ?? '#')
                                : $arsip->item_url ?? '#';
                        ...
                        <div class="arsip-card bg-white border border-[#E5D6BF] p-6 rounded-2xl shadow-sm flex flex-col justify-between hover:border-[#8D2B1D] hover:shadow-md transition-all duration-300 group"
                            data-title="{{ strtolower($title) }}" data-category="{{ $filterCat }}">
                            <div>
                                <!-- Header Kartu -->
                                <div class="flex justify-between items-center mb-4">
                                    <span
                                        class="bg-[#EFE4D3] text-[#8D2B1D] text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-md border border-[#C8A45A]/30">
                                        {{ $labelBadge }}
                                    </span>

                                    @if ($id && Route::has('pengguna.arsip.destroy'))
                                        <form action="{{ route('pengguna.arsip.destroy', $id) }}" method="POST"
                                            onsubmit="return confirm('Apakah Anda yakin ingin menghapus {{ $title }} dari simpanan?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="inline-flex items-center gap-1 text-xs font-bold text-red-600 hover:text-red-800 bg-red-50 hover:bg-red-100 px-2.5 py-1 rounded-lg border border-red-200 transition-colors"
                                ...
                                <!-- Konten Naskah -->
                                <h3 class="text-xl font-bold text-[#2B1A0E] mb-1 group-hover:text-[#8D2B1D] transition-colors"
                                    style="font-family: 'Cormorant Garamond', serif;">
                                    {{ $title }}
                                </h3>
                                <p class="text-xs font-medium text-[#8C7A65] mb-3">Penulis: {{ $penulis }}</p>
                                <p class="text-xs text-[#675A4D] leading-relaxed mb-4 line-clamp-3">{{ $deskripsi }}
                                </p>
                            </div>
                ...
                <div id="noResults"
                    class="hidden bg-white border border-[#E5D6BF] p-12 rounded-2xl text-center shadow-sm">
                    <div
                        class="w-16 h-16 bg-[#EFE4D3] text-[#8D2B1D] rounded-2xl flex items-center justify-center mx-auto text-3xl mb-4 border border-[#C8A45A]/30">
                        🔍</div>
                    <h3 class="text-2xl font-bold text-[#2B1A0E]" style="font-family: 'Cormorant Garamond', serif;">
                        Arsip Tidak Ditemukan</h3>
                    <p class="text-xs text-[#675A4D] mt-2 max-w-md mx-auto">Tidak ada arsip yang cocok dengan kata kunci
                        atau kategori yang Anda pilih.</p>
                </div>
            @else
                <div class="bg-white border border-[#E5D6BF] p-12 rounded-2xl text-center shadow-sm">
                    <div
                        ...
                        📚</div>
                    <h3 class="text-2xl font-bold text-[#2B1A0E]" style="font-family: 'Cormorant Garamond', serif;">
                        Belum Ada Koleksi Tersimpan</h3>
    ...
    <script>
        function filterArsip() {
            ...
            cards.forEach(card => {
                ...
                const matchesSearch = searchInput === '' || title.startsWith(searchInput) || title.includes(
                    searchInput);
